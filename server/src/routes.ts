...
export function createRoutes(d: RoutesDeps) {
    ...
    function placeOrder(
        ...
        const result = matching.placeOrder({
            id,
            owner,
            side: input.side,
            type: input.type,
         ...
         * 私有订单：accepted
         *
         * 只有经过 JWT 认证、且地址属于 owner 的 WS
         * 才能收到。
         */
        const acceptedOrder = result.resting ?? {
            id,
            owner,
            side: input.side,
            type: input.type,
        ...
        for (const fill of result.fills) {
             ...
             * taker
             */
            const takerOrder = matching.getOrder(fill.takerOrderId);
             ...
             * maker
             */
            const makerOrder = matching.getOrder(fill.makerOrderId);
        ...
        for (
            const maker of new Set(
                result.fills.map((f) => f.maker),
            )
        ) {
            if (maker !== owner) {
                pushBalance(maker);
    ...
    function cancelOrder(
        ...
        const order = matching.cancelOrder(
            owner,
            id,
        );
